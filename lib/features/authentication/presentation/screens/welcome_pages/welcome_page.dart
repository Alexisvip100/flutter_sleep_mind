...
import 'package:sleep_mind/features/authentication/presentation/screens/welcome_pages/step_cards.dart';
import 'package:sleep_mind/features/authentication/presentation/screens/welcome_pages/title_line_tile.dart';
import 'welcome_pages_data.dart';
import 'welcome_page_routes.dart';
...
class _WelcomePageState extends State<WelcomePage>
  ...
  void initState() {
    ...
    _c = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 6000), // fadeAnimationDuration
    );
    ...
    // Arranque suave
    Future.delayed(
      const Duration(milliseconds: 150), // fadeDelayDuration
      _c.forward,
    );
  }
    ...
    try {
      if (_currentPage < _pages.length - 1) {
        _pageController.nextPage(
          duration: const Duration(milliseconds: 300), // pageTransitionDuration
          curve: Curves.easeInOut,
        );
      } else {
        Navigator.of(context).pushReplacement(
          WelcomePageRoutes.navigateToLoginPage(context),
        );
      }
    } catch (e) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        color: (_currentPage == 2 || _currentPage == 3)
            ? const Color(0xFF142F4E) // primaryBackgroundColor
            : null,
      ),
    );
  }
        ...
        children: [
          SizedBox(
            ...
            child: Positioned.fill(
              child: WelcomePageImages.buildImageWithFallback(
                imagePath: WelcomePageImages.page4BackgroundImage,
                width: 0,
                height: 0,
                fit: BoxFit.cover,
                fallback: Container(
                  ...
                  child: const Center(
                    child: Icon(Icons.image, color: Colors.white54, size: 100),
                  ),
                ),
              ),
            ),
          ),
          WelcomePageImages.buildImageWithFallback(
            imagePath: WelcomePageImages.page3OverlayImage,
            width: 400,
            height: 400,
            fit: BoxFit.contain,
            fallback: Container(
              ...
              child: const Icon(Icons.handshake, color: Colors.white54, size: 80),
            ),
          ),
        ],
      ),
  ...
  Widget _buildPageContent(WelcomePageData pageData) {
    ...
    if (_isOverlayPage) {
      return Padding(
        padding: const EdgeInsets.only(top: 300.0), // overlayTopPadding
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
  ...
  Widget _circleButton({
    ...
    return Container(
      decoration: BoxDecoration(
        color: const Color.fromRGBO(122, 156, 198, 1),
